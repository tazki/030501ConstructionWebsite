...
{
    function imageUpload($image_filename_uploader, $cWidth = 0, $cHeight = 0, $suffix="_thumb", $path='repository/deals/')
    {
        $CI =& get_instance();
        ...
        {   
            $upload_config['upload_path'] = str_replace('hadmin', $path, $CI->config->config['base_path']);
...
if ( ! function_exists('listCategory'))
{
    function listCategory()
    {
        ...
        $rows = $ci->Base_model->list_all_by_field('tz_group', 'group_id', '', '', '', '', 'group_status = "2"');
        $prep_rows = array();            
        foreach ($rows as $key => $val)
        {
            $prep_rows['id_name_list'][$val['group_id']] = $val;
            $slug_url = str_replace(array(' ','&'), '-', strtolower($val['group_name'])).'-'.$val['group_id'];
            if($val['group_primary_id'] == 1)
            {
                $prep_rows['parent'][$val['group_id']] = $val;
                $prep_rows['parent'][$val['group_id']]['slug_url'] = $slug_url;
            }
            else
            {
                $prep_rows['child'][$val['group_primary_id']][$val['group_id']] = $val;
                $prep_rows['child'][$val['group_primary_id']][$val['group_id']]['slug_url'] = $slug_url;
                $prep_rows['child_id_list'][$val['group_id']] = $val['group_id'];
            }
        }
...
if ( ! function_exists('productUrl'))
{
    ...
    {
        $name = str_replace(array(' ','&','/','(',')','~'), '-', strtolower($name)).'-'.$id;
